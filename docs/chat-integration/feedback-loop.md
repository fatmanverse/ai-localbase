...
治理台同时支持查看 FAQ 发布历史、导出发布历史，以及一键继续发布到上次文档，减少重复找目标文档的操作。

知识库文档列表会直接标出 `默认 FAQ 合集` / `FAQ 文档`；如果同一 FAQ 曾经分散到多个文档，治理台会主动提示，方便运营收敛。
治理台还支持直接把 FAQ 发布到当前知识库的默认 FAQ 合集，并在导出发布历史时选择 Markdown 或 JSON，方便留档和后续对接脚本。
