...
`
	...
	documentReindexTaskResp := performRequest(
		t,
		engine,
		http.MethodPost,
		fmt.Sprintf("/api/knowledge-bases/%s/documents/%s/reindex-task", knowledgeBaseID, uploadResult.Uploaded.ID),
		nil,
		"",
	)
	if documentReindexTaskResp.Code != http.StatusAccepted {
		t.Fatalf("expected async document reindex status 202, got %d, body=%s", documentReindexTaskResp.Code, documentReindexTaskResp.Body.String())
	}
	var documentReindexTask model.DocumentUploadTask
	decodeJSONResponse(t, documentReindexTaskResp.Body.Bytes(), &documentReindexTask)
	if documentReindexTask.TaskType != "reindex" {
		t.Fatalf("expected async reindex task type reindex, got %s", documentReindexTask.TaskType)
	}
	if documentReindexTask.DocumentID != uploadResult.Uploaded.ID {
		t.Fatalf("expected async reindex task document id %s, got %s", uploadResult.Uploaded.ID, documentReindexTask.DocumentID)
	}
