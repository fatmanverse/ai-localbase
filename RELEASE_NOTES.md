...
- **去掉知识来源展示**：普通聊天页与嵌入式 widget 回复区不再直接展示“知识来源”，界面更干净，但后台来源元数据仍保留用于检索追踪、反馈归因与治理分析
- **新增治理台入口**：普通前端侧边栏新增“治理台”按钮，点击后会直接打开治理台，并默认切到 **反馈明细** 标签页，方便集中查看点踩结果与补充说明
- **治理台支持 URL 定位标签**：现在可通过 `?mode=ops&tab=feedback` 直接进入反馈明细，也会在切换标签时同步更新地址
- **图片话术改成人话**：把“当前检索结果未包含可直接利用的图片知识”这类内部化表达改成更自然的人工说明，避免把内部状态直接暴露给最终用户
...
### 前端性能收敛：一次性覆盖聊天区 / 侧边栏 / 知识库面板

- **聊天区**：将顶部栏、消息列表、消息气泡、快捷问题、输入区拆成记忆化子组件，输入框打字时不再反复拖着整段历史消息和 Markdown 渲染一起重刷
- **消息渲染**：Markdown 规范化与 Mermaid 渲染收敛到单条消息内部，复制、反馈展开、反馈填写等局部状态只影响对应消息
- **侧边栏**：会话列表改成行级别记忆化，重命名、菜单展开、当前会话消息数变化时，不再让整列会话项一起重算
- **知识库面板**：面板本体增加记忆化边界，并补充局部汇总缓存，减少无关状态变化带来的白跑
- **样式层**：为消息项、反馈区、知识库卡片、会话行增加 `contain / content-visibility`，进一步降低长列表场景下的布局与绘制成本
...
- 将设置弹层从 `Sidebar` 内部提升到 `App` 顶层渲染，避免侧边栏本地状态和会话列表交互把设置页一起拖慢
- `SettingsPanel` 改为记忆化导出，并配合稳定的 `onClose / onSave` 回调，减少上传进度、聊天状态等无关更新对设置页的干扰
- 侧边栏保留“打开设置”入口，但不再承担设置表单渲染职责，渲染边界更清晰，后续继续做侧边栏优化也不会波及设置页
- 这轮优化重点是外层渲染隔离，继续收敛设置页操作时的卡顿感
...
- 将设置表单进一步拆成可复用的记忆化字段组件，减少同一区块内“改一个字段、整块跟着重刷”的情况
- 聊天模型、Embedding 模型与 UI 文案区的输入事件统一改成稳定回调，降低高频输入和滑块拖动时的无效 diff 开销
- 设置页补齐文本域统一样式，并给字段容器增加局部布局隔离，长文本编辑与备用模型批量录入更顺手
- 这轮优化主要继续收敛设置页操作卡顿，不涉及本地服务启动联调
...
- 将设置页拆成 `聊天模型 / Embedding 模型 / UI 文案 / 底部操作区` 四个独立记忆化子组件，避免一个输入框变更触发整页表单重算
- 关键事件处理器统一改成稳定回调，减少子组件因函数引用变化带来的无效重渲染
- 保留延后比较草稿配置的策略，并把配置差异判断收敛到真正需要的区域，继续降低输入卡顿感
- 设置页现已更适合长文本编辑、滑块拖动和备用模型批量录入场景
...
- 移除设置弹层背景的高成本模糊渲染，减少输入、拖动滑块和滚动时的整页重绘
- 设置面板增加渲染隔离与滚动容器约束，降低长表单操作时的 repaint / reflow 开销
- 设置草稿的备用模型解析与配置差异比较改为延后计算，减少输入时的同步阻塞
- 顺手修复“默认问题建议”在设置页修改后未正确进入最终保存配置的问题
